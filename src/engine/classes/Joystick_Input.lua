...
local ANALOG_DEADZONE = 0.33
local ANALOG_THRESHOLD_HORIZ = 0.22
local ANALOG_THRESHOLD_VERT = 0.33
...
local default_conf = {
   capture_fns = {
      ...
      capture_axis_value = function(name, conf, state, src)
         local dz = conf.deadzone or 0
         local val = src:getGamepadAxis(conf.axis_name or name)
         return math.abs(val) > dz and val or 0
      end,
   ...
   buttons = {
      ...
      ["virt_up"] = {
         axis_name = "lefty",
         axis_threshold = -ANALOG_THRESHOLD_VERT,
         btn_name = "dpup",
      ...
      ["virt_left"] = {
         axis_name = "leftx",
         axis_threshold = -ANALOG_THRESHOLD_HORIZ,
         btn_name = "dpleft",
         capture_is_button_down = capture_virt_dir,
      },
      ["virt_right"] = {
   ...
   axii = {
      ["leftx"] = { deadzone = ANALOG_DEADZONE },
      ["lefty"] = { deadzone = ANALOG_DEADZONE },
      ["rightx"] = { deadzone = ANALOG_DEADZONE },
      ["righty"] = { deadzone = ANALOG_DEADZONE },
      ["triggerleft"] = {},
      ["triggerright"] = {},
   }
}
